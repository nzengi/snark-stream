<!--
  ...
  ------------------------
  This directory is a vendored fork of privacy-scaling-explorations/halo2 at
  commit 198e9ae (toolchain 1.82), kept here so the out-of-core measurement
  demo builds without a separate checkout. The only changes from upstream are
  the out-of-core SRS streaming and tiled quotient in halo2_backend, plus the
  `ooc-demo` crate; everything else is stock PSE-halo2. The exact diff against
  198e9ae is `../halo2-stream/integration/halo2-params-streaming.patch`.

  Build the demo:  cd halo2-fork && cargo build --release -p ooc-demo
  Upstream license (MIT OR Apache-2.0) is preserved below in LICENSE-*.
  Original PSE-halo2 README follows.
-->
